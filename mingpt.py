...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        print(f"forward() - y.shape:{y.shape} attn.shape:{attn.shape}")
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        print(f"VisionTransformer - img_size:{img_size} patch_size:{patch_size} in_chans:{in_chans} num_classes:{num_classes}")
        print(f"    embed_dim:{embed_dim} depth:{depth} num_heads:{num_heads} mlp_ratio:{mlp_ratio}")
        print(f"    qkv_bias:{qkv_bias} qk_scale:{qk_scale} drop_rate:{drop_rate} attn_drop_rate:{attn_drop_rate}")
        ...
        print(f"    type(self.patch_embed):{type(self.patch_embed)} num_patches:{num_patches}")
    ...
    def get_last_selfattention(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i < len(self.blocks) - 1:
                x = blk(x)
                print(f"(1)get_last_selfattention - x:{x.shape}")
            else:
                # return attention of the last block
                value = blk(x, return_attention=True)
                print(f"(2)get_last_selfattention value.shape:{value.shape}")
                return value
